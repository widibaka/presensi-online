...
// Awal program
session_start();
// Value awal session token, jika token bukan sama dengan 6 digit maka token = 0
// echo strlen($_SESSION['token']);
if(!isset($_SESSION['token']) or strlen($_SESSION['token']) !== 6){
	$_SESSION['token'] = 0;
}
...
if(isset($_POST['submit'])){
	...
	$sql = "SELECT * FROM riyousha0 WHERE namae = '$user' AND pasuwado = '$pass' LIMIT 1";
...
if($_SESSION['token'] !== 0){
	// Jika Token sedang aktif, maka redirect ke sesi aktif
	header("location: dashboard_sesi_aktif.php");
}
...
<br>
<form action="dashboard_sesi_aktif.php" method="post">
	<span>Kode Sesi Absensi:</span><br>
	<!-- <input type="text" name="token" value="<?php cetak_token(); ?>" disabled><br> -->
	<input type="hidden" name="id_dosen" value="<?php echo $id_dosen; ?>">
	<input type="hidden" name="token" value="<?php cetak_token(); ?>">
    <button type="submit" class="mb-4 btn btn-primary btn-icon-split">
      <span class="icon text-white-50">
      <i class="fas fa-smile"></i>
      </span>
      <span class="text">Buka Sesi Absensi</span>
    </button>
</form>
<br>
</p>
